...
bucket = "test"
org = "influxdata"
token = "OcRCCfoprWdjuTuEsPG5oQL4rDmp0eqF"
url = "http://192.168.0.113:30001"
...
def process_cu_xml(file_path):
    ...
    for measValue in root.findall('.//{http://www.3gpp.org/ftp/specs/archive/28_series/28.532#measData}measValue'):
        ...
        for meas_name, value in fields.items():
            point = Point("CU").tag("measObjLdn", meas_obj).field(meas_name, value)
...
def process_du_xml(file_path):
    ...
    for measValue in root.findall('.//{http://www.3gpp.org/ftp/specs/archive/28_series/28.532#measData}measValue'):
        ...
        # 逐个插入每个字段
        for meas_name, value in fields.items():
            point = Point("DU").tag("measObjLdn", meas_obj).field(meas_name, value)
            write_api.write(bucket=bucket, org=org, record=point)
